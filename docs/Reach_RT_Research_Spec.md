...
| 版 | 0.1.3 |
| 作成日 | 2026-09-18 |
| 更新日 | 2026-09-21：仮想ロボット・実映像接続・PTS対応修正への参照追加 |
| 状態 | G1-01～03の基盤を実装。画像による閉ループと研究性能検証は未完了 |
...
G1-02/03の実装、前提、検証結果は[G1-02/03実装説明](Reach_RT_G1_Robot_Video.md)、現在動く条件は[robot JSON](../config/reach_rt_g1_robot.json)を参照する。仮想カメラの実画素を実コーデック・UDPへ接続し、出力PTSと画像内番号で元の撮影情報を照合する。現段階の指令は時間で決める前進・停止であり、`robot_video_completed`を閉ループの作業成功と扱わない。実装の完了判定は[進捗表](Reach_RT_Implementation_Progress.md)で管理する。
