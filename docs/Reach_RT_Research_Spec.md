...
| 版 | 0.1.8 |
| 作成日 | 2026-09-18 |
| 更新日 | 2026-09-22：G2-01の双方向容量・直列化・有限キュー実装と検証を反映 |
| 状態 | G1通過、G2-01完了。実通信5ケース・単体・設定検査・回帰確認に合格。G2全体は未通過、次はG2-02。研究方式の性能比較は未着手 |
...
H1～H4を満たすことは未確認である。効果がゼロの条件、悪化する条件、実行不能な条件も成果に含める。少数の好都合な場面だけで全般的優位を主張しない。
...
G2-01で、上り映像と下りRCMDに独立した実UDP中継と容量FIFOを実装した。実装意図・構造・設定・検証結果は[G2-01説明](Reach_RT_G2_Link.md)。`link_model`を省略したG1経路は維持する。現段階は以下の手順2/3が対象で、共通予算（1）はG2-03、損失・ジッタ等（4/5）の時刻トレースはG2-02以降、状態通知の接続はG2-05で扱う。

実装上の定義：方向別のキュー占有は送信中を含む全IP長。FIFO・tail-drop、同時刻では完了解放を先に処理する。容量は時刻0からの区分一定bpsで、送信途中の変化を積分し、完了時刻を1 µs単位で切り上げる。容量0でも未処理量を保存し、再開後に継続する。モデル時刻と実sendtoの遅れを別に記録する。現在の中継で使う下りはRCMDのみで、未利用RNVP応答を上りへ反射させない。全プロトコルの予算計上完了を意味しない。
