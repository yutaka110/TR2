...
ActionExecution::ActionExecution(BudgetTransport& budget,const std::filesystem::path& path,NetworkManager* network,int task):budget_(budget),network_(network),task_(task),predict_(network!=nullptr){
    if(predict_){
...
uint64_t ActionExecution::Record(const DecisionSnapshot& s,const std::vector<ActionTicket>& cs,size_t selected,uint32_t id){
    ...
    }if(predict_)Predict(s,cs,d,id);return d;
}
...
void ActionExecution::Close(){std::lock_guard lock(mutex_);if(closed_)return;closed_=true;
