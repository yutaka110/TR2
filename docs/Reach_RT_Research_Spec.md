...
| 版 | 0.1.20 |
| 作成日 | 2026-09-18 |
| 更新日 | 2026-09-24：G4-02の有限窓予測・分離校正・範囲外・負の結果を反映 |
| 状態 | G0～G3通過、G4-02完了。受信済み情報に基づく候補別予測、学習8／校正4／別試行検証4／ストレス2を記録。校正改善は未確認。全体26/38、G4は2/5、研究ゲート4/7、次はG4-03。G4全体・提案方式Rの効果は未判定 |
...
初期実装は最新4画像のメタデータと、DEFER/FRESH/PROTECT g2・g4・g8/REPAIR/REFRESHの7候補。PROTECTは新規画像＋実XORの一組、REPAIRは受信済み選択再送要求の実チャンク、REFRESHは生成要求であり、出力確認後のIDR送信には改めて課金する。IDR要求の500 ms抑制、撮影後200 msの送信期限、2回の再送要求上限を扱う。候補段階の費用は予約ではなく、送信時の共通／方向別byte・レート制約で最終判定する。現在はイベント駆動の接続確認用固定選択で、20 ms周期や価値最適化は未実装。[構造・検証・段階的な具体化](Reach_RT_G4_Actions.md)を参照。
...
G4-03は20 ms周期・200 ms窓・最大64候補を継続し、一判断の計算打切りをINITIAL 5 msとする。p95≤2 ms・p99≤5 msは実測する目標で、Windows上の時間保証ではない。時間超過や範囲外では、現在の期限・予算で再検査済みの共通回復処理、またはDEFERへ移る。打切り後の超過も記録する。

G4-04では既存方式とのシステム比較と、同実行層での共通情報／復号追加／作業追加／両情報の寄与比較を分ける。H2は復号情報を保ったまま作業情報を除いて測る。同係数での比較と同じ調整機会で再調整した比較を区別する。G4-05で設定・主比較・試行数・無効条件・区間推定を固定する。詳細な構造・ログ・各工程の完了条件は[G4実装計画](Reach_RT_G4_Implementation_Plan.md)に定める。G4の実装は現時点で未着手。
...
現在はG4-02まで完了し、G4は2/5、全体26/38項目、研究ゲート4/7。予測単体37・実行層684・既存2,000検査、実測予測の分離検証とG4-01実UDP 7条件・B3・従来モードの回帰が合格。次はG4-03「ReachSchedulerと代替処理」。G4～G6は未通過。校正による改善・全18姿勢の再試験・Rの性能比較は認定しておらず、実装完了と研究効果の判定を分ける。
